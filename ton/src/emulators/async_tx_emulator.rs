...
enum Task {
    ...
    EmulateTicktock(TXEmulTickTockArgs),
}
...
pub struct AsyncTxEmulator {
    tx: Sender<InternalCmd>,
    _handler: thread::JoinHandle<TonResult<u64>>,
}
...
impl AsyncTxEmulator {
    ...
    pub async fn emulate_ord(&self, args: &TXEmulOrdArgs) -> EmulatorResult {
        self.execute_task(Task::EmulateOrd(args.clone())).await
    }

    pub async fn emulate_ticktock(&self, args: &TXEmulTickTockArgs) -> EmulatorResult {
        self.execute_task(Task::EmulateTicktock(args.clone())).await
    }
    ...
    async fn execute_task(&self, task: Task) -> EmulatorResult {
        ...
        rx.await.map_err(|e| TonError::Custom(format!("receive task error: {e}")))?
    }

    fn worker_loop(mut obj: TXEmulator, receiver: Receiver<InternalCmd>) -> TonResult<u64> {
        let mut counter = 0;
        loop {
            ...
            counter += 1;
            let result = match command.task {
                Task::EmulateOrd(args) => obj.emulate_ord(&args),
                Task::EmulateTicktock(args) => obj.emulate_ticktock(&args),
            };
...
mod tests {
    ...
    #[tokio::test]
    async fn test_async_tx_emulator_emulate_ord() -> anyhow::Result<()> {
        sys_tonlib_set_verbosity_level(0);
        let emulator = AsyncTxEmulator::new(0, false, None)?;
        ...
        assert_err!(emulator.emulate_ord(&ord_args).await);
        ord_args.emul_args.ignore_chksig = true;
        let response = assert_ok!(emulator.emulate_ord(&ord_args).await);
