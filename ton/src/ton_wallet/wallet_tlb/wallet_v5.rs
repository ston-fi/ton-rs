...
mod test {
    ...
    fn test_wallet_ext_msg_body_v5_order() -> anyhow::Result<()> {
        ...
        assert_eq!(Msg::from_cell(&body.msgs[0])?.info.as_int().unwrap().value.coins.to_u128(), 10000);
        assert_eq!(Msg::from_cell(&body.msgs[1])?.info.as_int().unwrap().value.coins.to_u128(), 10001);
        assert_eq!(Msg::from_cell(&body.msgs[2])?.info.as_int().unwrap().value.coins.to_u128(), 10002);
        assert_eq!(Msg::from_cell(&body.msgs[3])?.info.as_int().unwrap().value.coins.to_u128(), 10003);
