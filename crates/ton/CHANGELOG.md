...
- **Breaking:** `ContractClient::builder` now requires separate `StateProvider` and `EmulatorProvider` implementations. Native tonlib users should pass `TLStateProvider` and `TLEmulatorProvider`; `TLProvider` has been removed.
